...
class Dragon(Raid):
    def __init__ (self, parent:Notebook):
        super().__init__(raid_name="dragon",parent=parent)
        self.render_frame()
    
    def render_frame(self):
        # total count
        self.total_raid_image = resize_image(img=tk.PhotoImage(file=image_search("dragonCount.png", raid=self.raid_name)),
                                             newHeight=50,
                                             newWidth=50)
        ...
        self.trash = ButtonItem(frame=self.frame, 
                          image=tk.PhotoImage(file=image_search("dragonLitter.png", raid=self.raid_name)), 
                          name="trash")
        ...
        # earring
        self.earring = ButtonItem(frame=self.frame, 
                            image=tk.PhotoImage(file=image_search("dragonEarring.png", raid=self.raid_name)), 
                            name="earring")

        self.earring.button.grid(column=2,row=1)
        self.earring.button.bind('<Button-1>', lambda e : self.add_counter(self.earring))
        self.earring.button.bind('<Button-2>', lambda e : self.subtract_counter(self.earring))
        self.earring.button.bind('<Button-3>', lambda e : self.subtract_counter(self.earring))

        self.earring.count_label.grid(column=2,row=2)
        self.earring.percentage_label.grid(column=2,row=3)

        # sand
        self.sand = ButtonItem(frame=self.frame,
                         image=tk.PhotoImage(file=image_search("dragonSand.png", raid=self.raid_name)),
                         name="sand")
        ...
        self.add_items(self.trash,self.earring,self.sand)
        ...
        # anima count
        self.anima_image = resize_image(img=tk.PhotoImage(file=image_search("dragonAnima.png", raid=self.raid_name)),
                                        newWidth=25,
                                        newHeight=25)
