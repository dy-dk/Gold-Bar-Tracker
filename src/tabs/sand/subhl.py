...
class SUBHL(Raid):
    def __init__ (self, parent:Notebook):
        super().__init__(raid_name="subhl",parent=parent)
        self.render_frame()
    
    def render_frame(self):
        # total count
        self.total_raid_image = resize_image(img=tk.PhotoImage(file=image_search("subhlCount.png", raid=self.raid_name)),
                                             newWidth=50,
                                             newHeight=50)
        ...
        self.trash = ButtonItem(frame=self.frame, 
                          image=tk.PhotoImage(file=image_search("subhlLitter.png", raid=self.raid_name)), 
                          name="trash")
        ...
        self.sand = ButtonItem(frame=self.frame, 
                         image=tk.PhotoImage(file=image_search("subhlSand.png", raid=self.raid_name)), 
                         name="sand")
        
        self.sand.button.grid(column=3,row=1)
        ...
        self.sand.count_label.grid(column=3,row=2)
        self.sand.percentage_label.grid(column=3,row=3)
        
        # add all drop items -> determine which item contribute to total count
        self.add_items(self.trash,self.sand)

        # reset button
        self.reset_button = ttk.Button(self.frame, text="Reset", command=lambda:self.reset_count(), width=12).grid(padx= [102,0], column=4, row=5)

        # anima count
        self.anima_image = resize_image(img=tk.PhotoImage(file=image_search("subhlAnima.png", raid=self.raid_name)),
                                        newWidth=25,
                                        newHeight=25)
